...
func (s *Service) Submit(req Request) {
	...
	if s.dataService != nil && req.Comment.ParentID != "" {
		if p, err := s.dataService.Get(req.Comment.Locator, req.Comment.ParentID, store.User{}); err == nil {
			req.parent = p
			req.Emails = deduplicateStrings(s.getNotificationDetails(req, p, s.dataService.GetUserEmail))
			req.TelegramUsers = deduplicateStrings(s.getNotificationDetails(req, p, s.dataService.GetUserTelegram))
		}
	}
	select {
	case s.queue <- req:
...
// getNotificationDetails returns list of details (like email or telegram username) for users
// interested in notifications for provided comment.
// Details are not added to the returned list in case the original message is from the same user as the notification receiver.
func (s *Service) getNotificationDetails(
	req Request,
	notifyComment store.Comment,
	getUserDetail func(string, string) (string, error),
) (result []string) {
	...
	if notifyComment.ParentID != "" {
		if p, err := s.dataService.Get(req.Comment.Locator, notifyComment.ParentID, store.User{}); err == nil {
			result = append(result, s.getNotificationDetails(req, p, getUserDetail)...)
		}
	}
	return result
}
